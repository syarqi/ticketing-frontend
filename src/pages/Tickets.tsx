            ...
            {t.status === 'IN_PROGRESS' ? (
              <div className="flex items-center justify-between">
                <span className="text-xs text-slate-500">
                  👨‍🔧 {t.assignedTo?.fullName} — <span className="font-medium">IN PROGRESS</span>
                </span>
                <Link
                  to={`/tickets/${t.id}`}
                  className="text-xs font-semibold text-blue-600 border border-blue-200 rounded-md px-3 py-1.5 hover:bg-blue-50"
                >
                  DETAIL
                </Link>
              </div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="flex-1 text-center text-xs font-semibold text-white bg-green-600 hover:bg-green-700 disabled:opacity-60 rounded-md px-3 py-2"
                >
                  {takingId === t.id ? '...' : 'AMBIL'}
                </button>
              </div>
